...
function CriarCompra() {
  ...
  const [listaProdutos, setListaProdutos] = useState<any>(["Carregando..."]);
  ...
  function handleSubmit() {
    async function fetchPostSubmit() {
      ...
      const response: Response | void = await fetch(apiEndpoints.compra, {
        method: "POST",
      ...
      if (response && response.status === 200) {
        ...
        let idCompraCriada = jsonResponse.id_compra;
  ...
  return (
    <Box
      ...
      autoComplete="off"
    >
      <Typography variant="h3">Adicionar compra</Typography>
      <br />
        ...
        <FormControl size="medium" sx={{ m: 1, minWidth: 250 }}>
          ...
          >
            {listaProdutos.map((produto: any, index: any) => (
              <MenuItem value={Number(produto.id_produto)} key={produto.nome}>
                {produto.nome}
              </MenuItem>
            ))}
            {/* {listaProdutos
